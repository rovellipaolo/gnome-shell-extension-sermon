...
var getServices = () =>
    new Promise((resolve, reject) => {
        const command = Settings.shouldFilterSystemdUserServices()
            ? COMMAND_LIST_USER
            : COMMAND_LIST_ALL;
...
var startService = (id) =>
    _runCommandFromTemplate(
        Settings.shouldFilterSystemdUserServices()
            ? COMMAND_TEMPLATE_START_USER
...
var restartService = (id) =>
    _runCommandFromTemplate(
        Settings.shouldFilterSystemdUserServices()
            ? COMMAND_TEMPLATE_RESTART_USER
            : COMMAND_TEMPLATE_RESTART,
        id
    );
...
var stopService = (id) =>
    _runCommandFromTemplate(
        Settings.shouldFilterSystemdUserServices()
            ? COMMAND_TEMPLATE_STOP_USER
            : COMMAND_TEMPLATE_STOP,
        id
    );

var _runCommandFromTemplate = (commandTemplate, id) =>
    new Promise((resolve, reject) => {
        ...
        return CommandLine.executeAsync(command)
            ...
            .catch((_) => {
                Log.e(
                    LOGTAG,
                    `Systemd service "${id}" could not be ${message}!`
                );
                reject();
            });
    });
